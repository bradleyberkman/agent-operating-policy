...
When skill/provider evidence is supplied, validate it against `assets/harness-surface-evidence.schema.json` and carry it unchanged into both scanner outputs. Model each Claude and Codex surface independently. Every observed skill identity names its repository, local-root, system, symlink, or plugin origin; configured and exposed are separate states. A symlink records its target identity but is never traversed by the audit.
...
A provider-aware invocation receipt is narrower than a run trace. Validate each JSONL line with `scripts/harness_evidence.py validate-receipts` and `assets/skill-invocation-receipt.schema.json`. It may contain only the harness, provider identity, callable name, UTC timestamp, caller class, and schema version—never prompt text, tool arguments, tool payloads, results, secrets, customer identifiers, or production data. Collection is off by default and must not be wired into hooks by an audit change.

Use `scripts/harness_evidence.py report` only on supplied offline files. The report joins configuration, exposure, and invocation without network access. An invocation disproves “no caller,” but does not prove usefulness or acceptance, so an `INVESTIGATE` row may become `PROBATION`, never `KEEP`, from this receipt alone.
