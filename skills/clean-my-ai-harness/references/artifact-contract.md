...
- optional `harness-surface-evidence.json` — validated Claude/Codex repository, local-root, system, symlink, and plugin configuration/exposure evidence copied from the scanner inputs;
- optional `skill-invocation-receipts.jsonl` and offline usefulness report — minimal provider-aware caller evidence supplied to the audit, never prompts or tool payloads.
...
Invocation collection is not enabled by a report build. When separately authorized, receipt recording must be explicit, mode `0600`, and limited to the schema fields. An offline report may move an invoked `INVESTIGATE` row to `PROBATION`; only acceptance evidence can support a stronger usefulness ruling.
