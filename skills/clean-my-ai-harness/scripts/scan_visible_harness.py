...
from harness_evidence import EvidenceError, load_surface_evidence
...
def is_candidate(relative_path: Path, *, include_documents: bool = False) -> bool:
    ...
    if suffix in TEXT_SUFFIXES and (hinted_path or hinted_name):
        return True
    return suffix in EXECUTABLE_CONTROL_SUFFIXES and (
...
def _read_candidate(path: Path, maximum_bytes: int, expected: os.stat_result) -> tuple[bytes, bool]:
    # O_NOFOLLOW, and then re-check the descriptor we actually got.
    #
    # The caller's lstat() established that this path was a regular file and not a
    # symlink — but a plain open() follows symlinks, so anything swapped in between
    # those two calls would be read from wherever the link points, while the report
    # still declares "symlink_traversal": false. That is a scanner escaping the tree
    # it was pointed at and mislabelling the result, not merely a stale read.
    #
    # O_NOFOLLOW refuses a symlink outright at open time, and fstat on the resulting
    # descriptor confirms it is the same file lstat saw rather than a different
    # regular file put in its place. Both failures raise OSError, which the caller
    # already reports as a read-error — a candidate that changes mid-scan is skipped
    # and named, never silently read from the wrong place.
    ...
    try:
        ...
        if not stat.S_ISREG(actual.st_mode) or (actual.st_dev, actual.st_ino) != (
            ...
            raise OSError(errno.ESTALE, "candidate changed between stat and open")
...
def inspect_candidate(path: Path, relative_path: Path, maximum_bytes: int) -> tuple[dict[str, Any] | None, list[dict[str, str]]]:
    ...
    try:
        ...
        payload, grew_past_limit = _read_candidate(path, maximum_bytes, file_stat)
        if grew_past_limit:
            return None, [{
                "path": relative_path.as_posix(),
...
def scan_visible_harness(
    ...
    *,
    ...
    skill_surface_evidence = (
        load_surface_evidence(skill_evidence)
    ...
    scope = {
        ...
        "skill_surface_evidence": skill_surface_evidence,
    }
    normalized_map = {
        "schema_version": SCHEMA_VERSION,
        "scan_id": scan_id,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--skill-evidence",
        type=Path,
        help="Optional validated repository/local-root/system/symlink/plugin evidence for Claude and Codex.",
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    except (OSError, EvidenceError, ScanConfigurationError) as exc:
